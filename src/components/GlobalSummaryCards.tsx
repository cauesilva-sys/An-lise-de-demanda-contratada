...
interface GlobalSummaryCardsProps {
  ...
  onSelectCategory?: (category: 'ALL' | 'EXCEEDED' | 'WARNING' | 'HIGHEST_PEAK' | 'OK') => void;
  activeCategory?: 'ALL' | 'EXCEEDED' | 'WARNING' | 'OK' | 'HIGHEST_PEAK';
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      >
        ...
        {highest ? (
          <div key={`highest-data-${highest.usinaId}`}>
          ...
          <p key="empty-highest" className="text-xs text-slate-400 py-2">Nenhum dado coletado</p>
        )}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs transition-all">
        ...
        {selectedSummary ? (
          <div key={`selected-summary-${selectedSummary.usinaId}`}>
            <div className="flex items-center justify-between gap-1">
              ...
              {selectedSummary.status === 'EXCEEDED' ? (
                <span key="status-badge-exceeded" className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-600 text-white uppercase shadow-xs">
                  ULTRAPASSAGEM DETECTADA
                </span>
              ) : selectedSummary.status === 'WARNING' ? (
                <span key="status-badge-warning" className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-200 uppercase">
                  ALERTA
                </span>
              ) : (
                <span key="status-badge-regular" className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-200 uppercase flex items-center gap-0.5">
          ...
          <p key="empty-selected" className="text-xs text-slate-400 py-2">Selecione uma usina no filtro</p>
        )}
      </div>
    </div>
  );
